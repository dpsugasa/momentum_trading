...
from StarScream.Trades import Trade
from StarScream.Portfolio import Portfolio
from StarScream.stock_frame import StockFrame
...
class StarScream():
    ...
    def __init__(self,  paper_trading: bool = True) -> None:
        ...
        self.session: LocalTerminal = self._create_session()
        self.trades = {}
    ...
    def _create_session(self) -> LocalTerminal:
        """Start a new session.
        Creates a new session with the BBG Terminal API and logs the user into
        the new session.
        Returns:
        ----
        BBG Client running on local PC.
        """

        # Create a new instance of the client
        bbg_client = LocalTerminal
    ...
    def create_portfolio(self) -> Portfolio:
        ...
        # Assign the Client
        self.portfolio.bbg_client = self.session
    ...
    def grab_current_quotes(self) -> dict:
        ...
        # Grab the quotes.
        quotes = self.session.get_reference_data(sids=list(symbols),
                                                  flds="PX LAST"  ).as_frame()

        return quotes

    def grab_historical_prices(self, start: datetime, end: datetime, bar_size: int = 1,
                               event: str = 'trade', symbols: List[str] = None) -> List[dict]:
        ...
        LocalTerminal.get_intraday_bar(sid, event, start, end, interval=60).as_frame()        
        """
        ...
        for symbol in symbols:
            ...
            historical_prices_response = self.session.get_intraday_bar(
                sid=symbol,
                event=event,
                #period_type='day',
                start=start,
